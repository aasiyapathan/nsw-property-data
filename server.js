...
class NSWPropertyAPI {
  constructor(config = {}) {
    this.config = {
      ...
      githubUser: config.githubUser || 'aasiyapathan',
 ...
 async searchAddressFromGitHub(query, limit) {
  ...
  try {
    ...
    if (!masterIndex) throw new Error('Master index not found');

    const searchTerm = query.toLowerCase().trim();
    const normalizedSearchTerms = this.createSearchVariations(searchTerm);
    
    let matchingAddresses = new Set();
    
    // Priority search - exact matches first
    for (const searchVariation of normalizedSearchTerms) {
      const exactMatch = Object.keys(masterIndex).find(addr => 
        addr.toLowerCase() === searchVariation
      );
      
      if (exactMatch) {
        matchingAddresses.add(exactMatch);
        console.log('⚡ Exact match - fast path:', exactMatch);
            // ADD THIS OPTIMIZATION: For exact matches, skip other variations
      if (exactMatch.toLowerCase() === query.toLowerCase().replace(/\s+/g, ' ').trim()) {
      console.log('⚡ Perfect match - using fast path');
      break; // Stop searching other variations for exact matches
      }
    }
    
    // If no exact match, do broader search but limit results
    if (matchingAddresses.size === 0) {
      for (const searchVariation of normalizedSearchTerms) {
        const searchWords = searchVariation.split(/\s+/).filter(word => word.length > 0);
        
        if (searchWords.length > 1) {
          const wordMatches = Object.keys(masterIndex)
            .filter(addr => {
              const lowerAddr = addr.toLowerCase();
              return searchWords.every(word => lowerAddr.includes(word));
            })
            .slice(0, 20); // Limit for performance
          
          wordMatches.forEach(addr => matchingAddresses.add(addr));
          
          if (matchingAddresses.size >= 10) break; // Stop early if we have enough
        }
      }
    }
	}
    ...
    for (const address of matchingAddressArray) {
      const availableYears = Object.keys(masterIndex[address]);
      const sortedYears = availableYears
        .map(y => parseInt(y))
        .sort((a, b) => b - a);
      
      // Only check most recent year for performance
      const mostRecentYear = sortedYears[0];
      
      try {
        const properties = await this.loadPropertiesForAddressYear(address, mostRecentYear.toString());
        ...
        if (properties && properties.length > 0) {
          ...
          results.push(...sortedProperties);
          
          if (results.length >= limit) break; // Stop when we have enough
        }
      } catch (error) {
        continue;
      }
    ...
    // Cache results for longer if it's a specific address search
    const cacheTime = matchingAddresses.size === 1 ? 30 * 60 * 1000 : 10 * 60 * 1000;
  ...
  async searchPriceFromGitHub({ minPrice, maxPrice, suburb, year, limit }) {
    ...
    try {
      ...
      for (const searchYear of years.slice(0, 10)) { // Limit years
...
async loadPropertiesForAddressYear(address, year) {
  try {
    ...
    if (addressData) {
      const filtered = addressData.properties
    ...
    // SIMPLE VERSION - CHECK ALL CHUNKS
    for (const chunk of manifest.chunks) {
      const chunkData = await this.fetchGitHubFile(year + '/' + chunk.filename);
      if (!chunkData) continue;

      const matches = chunkData.properties.filter(p => {
        const propAddress = (p.a || p.address || '').toString();
        const searchAddress = address.toString();
        
        if (!propAddress || !searchAddress) return false;
        
        const normalizedProp = this.normalizeAddressForMatching(propAddress);
        const normalizedSearch = this.normalizeAddressForMatching(searchAddress);
...
async searchSingleChunk(year, chunk, address) {
  try {
    const chunkData = await this.fetchGitHubFile(year + '/' + chunk.filename);
    if (!chunkData) return [];
    ...
    const matches = chunkData.properties.filter(p => {
      const propAddress = (p.a || p.address || '').toString();
      if (!propAddress) return false;
      
      return this.normalizeAddressForMatching(propAddress) === 
             this.normalizeAddressForMatching(address);
    });

    return matches.length > 0 ? matches.map(p => this.decompressProperty(p)) : [];
  } catch (error) {
    return [];
  }
}

 // SAFE OPTIMIZATION 1: Enhanced Caching (replace fetchGitHubFile function)
...
async fetchGitHubFile(filepath) {
  ...
  const url = 'https://raw.githubusercontent.com/' + this.config.githubUser + '/' + 
              this.config.githubRepo + '/' + this.config.githubBranch + '/' + filepath;
    ...
    https.get(url, (response) => {
      ...
      response.on('end', () => {
        try {
          ...
          // IMPROVED: Smart caching based on file type
          let cacheTime = 15 * 60 * 1000; // 15 minutes default
          
          if (filepath.includes('master-address-index')) {
            cacheTime = 2 * 60 * 60 * 1000; // 2 hours for master index (rarely changes)
          } else if (filepath.includes('manifest')) {
            cacheTime = 60 * 60 * 1000; // 1 hour for manifests (rarely change)
          } else if (filepath.includes('properties-')) {
            cacheTime = 30 * 60 * 1000; // 30 minutes for property chunks
          }
  ...
  console.log('🏥 Checking GitHub data availability...');
  try {
    const masterIndex = await this.fetchGitHubFile('master-address-index.json');
    
    if (masterIndex) {
      const addressCount = Object.keys(masterIndex).length;
      console.log('✅ GitHub data available - ' + addressCount + ' addresses found');
      return {
        available: true,
      ...
      console.log('❌ GitHub data not available - master index returned null');
      return {
        available: false,
        addresses: 0,
        error: 'Master index file not accessible',
        lastCheck: new Date().toISOString()
      };
    }
  } catch (error) {
    console.log('❌ GitHub availability check error:', error.message);
    return {
      available: false,
      error: error.message,
      lastCheck: new Date().toISOString()
  ...
  createSearchVariations(searchTerm) {
  const variations = new Set();
  variations.add(searchTerm); // Original term
  ...
  const abbreviationMap = {
    ...
    'tce': ['terrace', 'ter', 'terr'],
    'ter': ['terrace', 'tce', 'terr'],
    'terr': ['terrace', 'tce', 'ter'],
  ...
  // Generate variations by replacing each word
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (abbreviationMap[word]) {
      // Create variations with each possible replacement
      abbreviationMap[word].forEach(replacement => {
...
normalizeAddressForMatching(address) {
  return address
    .toString()
    ...
    .replace(/\b(street|st|str)\b/g, 'st')     // Normalize street variations
    .replace(/\b(avenue|ave|av)\b/g, 'ave')    // Normalize avenue variations
    .replace(/\b(road|rd|r)\b/g, 'rd')         // Normalize road variations
    .replace(/\b(drive|dr|drv)\b/g, 'dr')      // Normalize drive variations
    .replace(/\b(place|pl|plc)\b/g, 'pl')      // Normalize place variations
    .replace(/\b(court|ct|crt)\b/g, 'ct')      // Normalize court variations
    .replace(/\b(close|cl|cls)\b/g, 'cl')      // Normalize close variations
    .replace(/\b(crescent|cres|cr|crs)\b/g, 'cr') // Normalize crescent variations
    .replace(/\b(circuit|cct|circ)\b/g, 'cct') // Normalize circuit variations
    .replace(/\b(lane|ln|la)\b/g, 'ln')        // Normalize lane variations
    .replace(/\b(terrace|tce|ter|terr)\b/g, 'tce') // Normalize terrace variations
    .replace(/\b(parade|pde|prd)\b/g, 'pde')   // Normalize parade variations
    .replace(/\b(boulevard|blvd|bvd)\b/g, 'blvd') // Normalize boulevard variations
    .replace(/\b(highway|hwy|hw)\b/g, 'hwy')   // Normalize highway variations
    .replace(/\b(square|sq|sqr)\b/g, 'sq')     // Normalize square variations
    .replace(/\b(grove|gr|grv)\b/g, 'gr')      // Normalize grove variations
    .replace(/\b(gardens|gdns|gdn)\b/g, 'gdns') // Normalize gardens variations
    .replace(/\b(esplanade|esp|espl)\b/g, 'esp') // Normalize esplanade variations
    .trim();
}
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .error { background: #f8d7da; border: 1px solid #f5c6cb; color: #721c24; padding: 1rem; border-radius: 10px; margin-bottom: 1rem; }
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="search-box">
            <input type="text" id="searchInput" class="search-input" placeholder="Enter property address (e.g., 'rawson street', 'george street', 'parramatta')">
    ...
    <script>
        ...
        async function checkStatus() {
            try {
                document.getElementById('statusText').innerHTML = 'Checking API...';
                const response = await fetch('/api/health');
                const data = await response.json();
                
                const statusText = document.getElementById('statusText');
                if (data.githubData && data.githubData.available) {
                    statusText.innerHTML = '✅ Ready - ' + data.githubData.addresses.toLocaleString() + ' addresses available';
                } else {
                    statusText.innerHTML = '⚠️ Data not available';
                }
            } catch (error) {
        ...
        async function searchProperties() {
            ...
            if (query.length < 3) {
                results.innerHTML = '<div class="error"><p>Please enter at least 3 characters</p></div>';
                return;
            }
            
            results.innerHTML = '<div class="loading">🔍 Searching for "' + query + '"...</div>';
            ...
            try {
                ...
                if (data.results && data.results.length > 0) {
                    let html = '<h3 style="margin-bottom: 1rem; color: #2c3e50;">Found ' + data.count + ' results</h3>';
                    
                    data.results.forEach(property => {
                        html += '<div class="property">';
                        html += '<div class="property-address">' + (property.address || 'N/A') + '</div>';
                        html += '<div class="property-details">';
                        html += '<div class="detail-item"><div>Sale Price</div><div class="price">$' + (property.salePrice ? property.salePrice.toLocaleString() : 'N/A') + '</div></div>';
                        html += '<div class="detail-item"><div>Sale Date</div><div class="detail-value">' + (property.saleDate || 'N/A') + '</div></div>';
                        html += '<div class="detail-item"><div>Suburb</div><div class="detail-value">' + (property.suburb || 'N/A') + '</div></div>';
                        html += '<div class="detail-item"><div>Property Type</div><div class="detail-value">' + (property.propertyType || 'N/A') + '</div></div>';
                        html += '<div class="detail-item"><div>Area</div><div class="detail-value">' + (property.area ? property.area + 'm²' : 'N/A') + '</div></div>';
                        html += '<div class="detail-item"><div>Postcode</div><div class="detail-value">' + (property.postcode || 'N/A') + '</div></div>';
                        html += '</div></div>';
                    });
                    
                    results.innerHTML = html;
                } else {
                    results.innerHTML = '<div class="property"><p>No properties found for "' + query + '". Try a different search term.</p></div>';
                }
            } catch (error) {
                results.innerHTML = '<div class="error"><p>Error: ' + error.message + '</p></div>';
            }
        }
...
if (require.main === module) {
  const config = {
    githubUser: process.env.GITHUB_USER || 'aasiyapathan',
